...
<Tabs items={["Client", "Server"]} defaultIndex="1">
  <Tabs.Tab>
    ```lua copy ---@param any local identifier = LGF.Core:GetIdentifier() ```
  </Tabs.Tab>
  <Tabs.Tab>
    ```lua copy ---@param source local identifier =
    LGF.Core:GetIdentifier(source) ```
  </Tabs.Tab>
</Tabs>
