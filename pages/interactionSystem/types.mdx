...
---@field vehicle number|nil -- Unique ID for the vehicle (only for addInteractionVehicle)
---@field vehicleBone number|nil -- Vehicle Bone (only for addInteractionVehicle)
---@field netID number|nil -- netId of the Entity (only for addInteractionEntity)
...
---@field nearby fun(interacionid:string): void|nil -- Function to execute when the player is near the interaction
